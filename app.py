...
st.set_page_config(
    ...
    page_icon="https://cdn-icons-png.flaticon.com/512/5328/5328114.png", 
    layout="wide",
...
with st.sidebar:
    ...
    st.write("**Developed By:** Tikam")

# ==================== 🎯 TABS STRUCTURE FOR GOOGLE APPROVAL ====================
tab1, tab2, tab3 = st.tabs(["🗂️ Services Directory", "⚖️ Privacy Policy & Terms", "📝 About & Disclaimer"])

with tab1:
    # --- Pura Main App Content Is Tab Me Chalega ---
    services_data = {
    ...
    search_query = st.text_input("🔍 Search exactly what you need (e.g., 'Tender', 'License', 'PAN', 'Kathmandu')...", "").strip().lower()
    found_any = False
...
with tab2:
    ...
    st.write("""
    ### 1. Data Collection
    This application is an open-source navigational directory. We **strictly do not collect, store, or log** any personal identifiable information (PII), biometric logs, application form parameters, or credential inputs. All citizen processing data remains isolated between your machine and the target government servers.
    
    ### 2. Third-Party Advertisement Links
    We reserve the right to utilize advertising network frameworks in the future. These networks may use cookies to serve contextual advertisements based on user navigation trends.
    
    ### 3. External Redirections
    All internal buttons utilize explicit hyperlinks directing structural traffic to official Nepal Government domain structures terminating strictly in `.gov.np` or trusted public corporation nodes. We are not liable for transient timeouts or data practices hosted on external server systems.
    """)
...
with tab3:
    ...
    st.write("""
    ### About The App
    **Nepal Government Services Directory** is an independent, non-profit technical hub designed and optimized by **Tikam**. The vision is to streamline public access by consolidating 56 disparate municipal, licensing, and recruitment nodes into a single responsive mobile matrix.
    
    ### ⚠️ Legal Disclaimer (Fair Use Notice)
    1. **Non-Affiliation:** This application is **NOT affiliated, authorized, endorsed by, or in any way officially connected** with the Government of Nepal, any provincial state ministry, or their respective departments. 
    2. **Official Data:** The official emblem and operational platform endpoints are utilized strictly under descriptive public informational reference criteria. All copyrights belong to their respective statutory owners.
    3. **No Warranty:** While we dynamically audit links, we hold no liability for service updates, exam form errors, or system issues occurring on official government portals.
    """)
